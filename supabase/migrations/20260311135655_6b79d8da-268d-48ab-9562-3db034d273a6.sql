...
-- Function 1: Start trainer trial (14 days + 3 day grace)
...
BEGIN
  ...
  INSERT INTO trainer_platform_subscriptions (
    ...
    p_trainer_id, 'trial', 'trial', 0,
    CURRENT_DATE, CURRENT_DATE + 14, CURRENT_DATE + 17,
    true, 0, 3,
    'not_required'
  )
  RETURNING *;
...
-- Function 2: Create trainer subscription (monthly=30d, annual=365d + 3d grace)
...
BEGIN
  ...
  INSERT INTO trainer_platform_subscriptions (
    ...
    p_trainer_id, p_plan_type, 'active', v_amount,
...
-- Function 3: Renew trainer subscription
...
BEGIN
  ...
  -- Get trainer_id and verify ownership
  ...
  -- If active, extend from current end_date; if expired, start from today
  IF p_is_active THEN
